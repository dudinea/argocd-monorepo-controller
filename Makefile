...
TEST_TOOLS_IMAGE=argocd-monorepo-controller-test-tools
...
$(error IMAGE_NAMESPACE must be set when IMAGE_REGISTRY is set (e.g. IMAGE_NAMESPACE=argoproj-labs))
endif
else
ifdef IMAGE_NAMESPACE
...
IMAGE_REPOSITORY=argocd-monorepo-controller
endif
...
.PHONY: codegen-local
#codegen-local: mod-vendor-local mockgen gogen protogen clientgen openapigen clidocsgen actionsdocsgen manifests-local notification-docs notification-catalog clidocsgen
codegen-local: mod-vendor-local mockgen gogen protogen clientgen  manifests-local 
	rm -rf vendor/
...
.PHONY: repo-server
repo-server:
	CGO_ENABLED=${CGO_FLAG} GODEBUG="tarinsecurepath=0,zipinsecurepath=0" go build -v -ldflags '${LDFLAGS}' -o ${DIST_DIR}/argocd-monorepo-repo-server ./cmd

.PHONY: controller
controller:
	CGO_ENABLED=${CGO_FLAG} GODEBUG="tarinsecurepath=0,zipinsecurepath=0" go build -v -ldflags '${LDFLAGS}' -o ${DIST_DIR}/argocd-monorepo-controller ./cmd
...
gen-docs-local: dist/argocd
	...
	hack/generate_command_ref.py "ARGOCD_BINARY_NAME=argocd-monorepo-repo-server ./dist/argocd  --help"  cmd/argocd-repo-server/commands/argocd_repo_server.go ./util/cache/cache.go ./reposerver/cache/cache.go > docs/monorepo_repo_server_conf.md

.PHONY: update-helm-docs-local
