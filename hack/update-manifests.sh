...
IMAGE_REPOSITORY="${IMAGE_REPOSITORY:-argocd-monorepo-controller}"

# Apply defaults if needed
if [[ -n $IMAGE_REGISTRY ]];then
    if [[ -z $IMAGE_NAMESPACE ]]; then
	echo "IMAGE_NAMESPACE must be set when IMAGE_REGISTRY is set (e.g. IMAGE_NAMESPACE=argoprojlabs)" >&2
	exit 1
    fi
...
if [ -n "$DETECTED_IMAGE" ] && [ "$DETECTED_IMAGE" != "quay.io/argoproj/argocd" ]; then
  ...
  SOURCE_IMAGE_NAME="quay.io/argoprojlabs/argocd-monorepo-controller"
...
if [ "$DETECTED_IMAGE" != "quay.io/argoprojlabs/argocd-monorepo-controller" ] && [ -n "$DETECTED_IMAGE" ]; then
